...
public class ControllerOwnerView extends Controller {
    ...
    void showYachts(ActionEvent event) {
        ...
        if (1 == showItems(Launch.getFac().getYachtsData(), event)) {
            yachtProp = "search";
            addYacht(event);
    ...
    void showClients(ActionEvent event) {
        ...
        if (1 == showItems(Launch.getFac().getClientsData(), event)) {
            clientProp = "search";
             manageClient(event);
...
//    }
    ...
    void addYacht(ActionEvent event) {
        if ("search".equals(yachtProp)) {
            ...
            updateList(Launch.getFac().searchYachts(data));
        ...
        System.out.println(Launch.getFac().getYachts());
        ...
        switch (yachtProp) {
            case "add":
                Launch.getFac().addYacht(data);
                break;
            case "modify":
                Launch.getFac().modifyYacht(yachtModProp.getValue(), data);
                break;
            case "remove":
                Launch.getFac().deleteYacht(data[1]);
                break;

        }

        updateList(Launch.getFac().getYachtsData());
        idBox.clear();
        nameBox.clear();
        typeBox.clear();
        lengthBox.clear();
    ...
    void manageClient(ActionEvent event) {
         ...
         if ("search".equals(clientProp)) {
            ...
            updateList(Launch.getFac().searchClients(data));
            return;
        }
        ...
        System.out.println(Launch.getFac().getClients());
        String[] data = {"1", idBoxClient.getText(), nameBoxClient.getText(), surnameBoxClient.getText(), phoneBox.getText()};
        System.out.println(Arrays.toString(data));
        switch (clientProp) {
            case "add":
                Launch.getFac().addClient(data);
                break;
            case "modify":
                Launch.getFac().modifyClient(clientModProp.getValue(), data);
                break;
            case "remove":
                Launch.getFac().deleteClient(data[1]);
                break;
        }
        updateList(Launch.getFac().getClientsData());
        idBoxClient.clear();
        nameBoxClient.clear();
